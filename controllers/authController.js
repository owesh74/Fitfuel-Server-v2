...
exports.verifyOtp = async (req, res) => {
    ...
    try {
        ...
        if (user.otp !== otp || user.otpExpires < Date.now()) { return res.status(400).json({ msg: 'Invalid or expired OTP.' }); }
        user.verified = true;
        user.otp = undefined;
...
exports.login = async (req, res) => {
  ...
  try {
    let user = await User.findOne({ email });
    if (!user || !user.verified) { return res.status(400).json({ msg: 'Invalid credentials or user not verified' }); }
...
exports.updateUserProfile = async (req, res) => {
  try {
    const { height, weight, age, gender, activityLevel, goal, goalWeight, goalDuration } = req.body;
...
exports.resetPassword = async (req, res) => {
  ...
  try {
    ...
    if (!user) { return res.status(400).json({ msg: 'Invalid OTP or OTP has expired.' }); }
    const salt = await bcrypt.genSalt(10);
